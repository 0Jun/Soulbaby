...
void Container::AddObjects()
{
    ...
    if(Queue.size() > 0)
    {
        ...
        for(QueueIterator QI = Queue.begin(); QI != Queue.end(); QI++)
        {
            ...
            if((QI)->properties["itemType"] == "Textbox")
            {
            ...
            if((QI)->properties["itemType"] == "BattleBackground")
            {
            ...
            if((QI)->properties["itemType"] == "LevelManager")
            {
            ...
            if((QI)->properties["itemType"] == "DamageSpark")
            {
            ...
            else if((QI)->properties["itemType"] == "Gib"){
                
                std::unique_ptr<Entity::Gib> ptr(new Entity::Gib);
                ptr->objectSprite.setPosition(stoi((QI)->properties["PosX"]),stoi((QI)->properties["PosY"]));
                //ptr->objectSprite.setTextureRect((QI)->textureRect);
                //ptr->gibTexture = ((QI)->texturePtr);
                ptr->vel.x = rand() % 12 + -6;
                ptr->vel.y = rand() % 3;
                ObjectContainer.push_back(std::move(ptr));
...
void Container::DrawObjects(sf::RenderTarget& window, class World& world)
{
    ...
    //if we have 3 containers, we could create iterators here and combine them to draw all objects; update & checkcollsion would be much more clear; still need diff zone containers
    ...
    if(ObjectContainer.size() > 0)
    {
        ...
        for (ObjectIterator it = ObjectContainer.begin(); it !=  ObjectContainer.end(); it++)
        {
            ...
            {
                (*it)->DrawShadow(World::GetInstance()->windowTexture);
                (*it)->Draw(World::GetInstance()->windowTexture);
            }
